...
import { useCoverageComparison } from '../hooks/useCoverageComparison'
...
const {
  historicalId: compareId,
  inlineError: compareError,
  comparison,
  loading: comparisonLoading,
  storeError: comparisonStoreError,
} = useCoverageComparison(selected)
    ...
    <template v-if="selected">
      ...
      <div class="coverage-grid">
        ...
        <aside class="coverage-evidence">
          ...
          <div class="compare-tools"><GitCompare :size="16" /><el-select v-model="compareId" placeholder="选择同场景历史版本，对比其到当前版本的差异" clearable><el-option v-for="item in comparable" :key="item.id" :label="compareOptionLabel(item)" :value="item.id" /></el-select></div>
          <p v-if="selected && !comparable.length" class="compare-hint">同场景暂无其他已完成评估，可在有多个版本后对比版本差异。</p>
          <EvaluationComparisonPanel
            :comparison="comparison"
            :loading="comparisonLoading"
            :error="compareError || comparisonStoreError"
          />
